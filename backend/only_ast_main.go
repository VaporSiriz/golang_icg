...
func AnalysisAst(body interface{}) string {
	defer func() {
	...
	f, err = parser.ParseFile(fs, "", body, parser.AllErrors)
	if err != nil {
		log.Printf("could not parse %s: %v", body, err)
	}
	...
	astAnalyzer.Init("goFile.go", fs)
	errStr := astAnalyzer.Analysis(f, info)
	fmt.Printf(errStr)
	return errStr
}
